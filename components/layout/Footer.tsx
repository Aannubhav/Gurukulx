    ...
    <footer className="border-t border-[#E5E5E5] bg-[#F7F7F7]">
      ...
      <div className="container-custom py-16">
        <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-8 lg:gap-10">
          ...
          <div className="col-span-2 sm:col-span-2 md:col-span-3 lg:col-span-2">
            ...
            <div className="flex items-center gap-3">
              {socialLinks.map(({ icon: Icon, label, href }) => (
                <a
                  ...
                  href={href}
                  aria-label={label}
                  className="w-9 h-9 rounded-lg border border-[#E5E5E5] bg-white flex items-center justify-center text-[#555555] hover:text-[#FF6A2A] hover:border-[rgba(255,106,42,0.3)] hover:shadow-[0_2px_12px_rgba(255,106,42,0.15)] transition-all duration-200"
                >
                  <Icon size={16} />
                </a>
              ))}
            ...
            {/* Newsletter */}
            <div className="mt-6">
              <p className="text-xs text-[#555555] mb-2 font-medium">Stay in the loop</p>
              <form className="flex gap-2" onSubmit={(e) => e.preventDefault()}>
                <input
                  type="email"
                  placeholder="you@institute.com"
                  className="flex-1 min-w-0 bg-white border border-[#E5E5E5] rounded-lg px-3 py-2 text-sm text-[#000000] placeholder-[#555555]/60 focus:outline-none focus:border-[rgba(255,106,42,0.5)] transition-colors"
                />
                <button
                  type="submit"
                  className="px-3 py-2 bg-[#FF6A2A] text-white text-xs font-bold rounded-lg hover:bg-[#FF8547] transition-colors shrink-0"
                >
                  Subscribe
                </button>
              </form>
          ...
          {Object.entries(footerLinks).map(([category, links]) => (
            <div key={category}>
              <h3 className="text-xs font-semibold tracking-wider uppercase text-[#555555] mb-4">
                {category}
              </h3>
              <ul className="space-y-3">
                {links.map((link) => (
                  <li key={link}>
                    <a
                      href="#"
                      className="text-sm text-[#555555] hover:text-[#FF6A2A] transition-colors"
                    >
                      {link}
                    </a>
                  </li>
      ...
      <div className="border-t border-[#E5E5E5]">
        <div className="container-custom py-5 flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <p className="text-xs text-[#555555]/50">
